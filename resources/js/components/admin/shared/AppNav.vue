...
export default {
  ...
  methods: {
    ...
    navigation() {
      this.items = [
        [
          { title: 'Профиль', icon: 'person', to: {name: 'profile'}, exact: false },
          { title: 'Задачи', icon: 'task', to: {name: 'task'}, exact: false },
        ],
        [
          { title: 'Выйти', icon: 'power_settings_new', action: this.logout }
        ]
      ]
    }
  }
